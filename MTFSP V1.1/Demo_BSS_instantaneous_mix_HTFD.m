...
Na      = 17;               % Number of Auto-terms matrices used in JD
Nx      = 1;                % Number of Cross-terms matrices used in JAD
th      = 1.7;              % Thereshold for Auto-terms and Cross-terms selection
...
TFD_X = cell(1,m);
for mm = 1:m
    TFD_X{1,mm} = Xckd(X(mm,:), X(mm,:), 1, 0.1, 0.1, M);
end
...
if(opt == 'y' || opt == 'Y')
    print(1,'Inst_BSS_HTFD_Fig1','-depsc');
    print(2,'Inst_BSS_HTFD_Fig2','-depsc');
else
    return
end
