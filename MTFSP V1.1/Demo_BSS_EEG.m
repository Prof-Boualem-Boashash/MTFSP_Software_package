...
f = 0:fs/(2*N-1):fs/2;
tfr = 4; RR = 18;
TFD_original = Xckd(s(RR,:), s(RR,:),1,0.05,0.1, N);
TFD_corrupted = Xckd(x(RR,:), x(RR,:),1,0.05,0.1, N);
TFD_estimated = Xckd(s_hat(RR,:), s_hat(RR,:),1,0.05,0.1, N);

%% Plotting
figure('Color',[1 1 1],'Position',[100, 80, 550 450]);
flatwf(f,t(1:tfr:end),abs(TFD_original(:,1:tfr:end)'),'w','k');
set(gca,'fontweight','bold','Fontsize',16);
ylabel('Time (s)','Fontsize',18); xlabel('Frequency (Hz)','Fontsize',18);

figure('Color',[1 1 1],'Position',[100, 80, 550 450]);
flatwf(f,t(1:tfr:end),abs(TFD_corrupted(:,1:tfr:end)'),'w','k');
set(gca,'fontweight','bold','Fontsize',16);
ylabel('Time (s)','Fontsize',18); xlabel('Frequency (Hz)','Fontsize',18);

figure('Color',[1 1 1],'Position',[100, 80, 550 450]);
flatwf(f,t(1:tfr:end),abs(TFD_estimated(:,1:tfr:end)'),'w','k');
set(gca,'fontweight','bold','Fontsize',16);
ylabel('Time (s)','Fontsize',18); xlabel('Frequency (Hz)','Fontsize',18);
...
if(opt == 'y' || opt == 'Y')
    print(1,'Clean_TFD','-depsc');
    print(2,'Corrupted_TFD','-depsc');
    print(3,'Filtered_TFD','-depsc');
    print(4,'BSS_EEG_Fig1','-depsc');
    print(5,'BSS_EEG_Fig2','-depsc');
else
    return
end
